...
const ProductDetailPage = () => {
  ...
  const handleAddToCart = async () => {
    ...
    console.log('Auth check:', { isAuthenticated, user, jwt: !!localStorage.getItem('jwt') });
    ...
    console.log('✅ JWT present:', token.substring(0, 20) + '...');
            ...
            <div>
              ...
              <div className="mb-6">
                ...
                {discountPercentage > 0 && (
                  <p className="text-green-600 font-medium mt-1">
                    You save LKR {(product.mrpPrice - product.sellingPrice).toLocaleString()} ({discountPercentage}% off)
                  </p>
                )}
              </div>
              ...
              {!isAuthenticated && (
                <div className="mb-4 p-3 rounded-lg bg-yellow-50 text-yellow-800 border border-yellow-200">
                  <p className="text-sm">
                    Please <button onClick={() => navigate('/login')} className="underline font-medium">log in</button> to add items to your cart
                  </p>
                </div>
              )}
              ...
              <button
                ...
                className="w-full btn-primary flex items-center justify-center py-3 text-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-[1.02]"
              >
                {addingToCart ? (
