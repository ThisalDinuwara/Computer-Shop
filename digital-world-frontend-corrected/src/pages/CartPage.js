                  ...
                  <div className="flex items-center mt-2">
                    ...
                    {item.mrpPrice > item.sellingPrice && (
                      <span className="ml-2 text-sm text-gray-500 line-through">
                        LKR {item.mrpPrice?.toLocaleString()}
                      </span>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 sticky top-24">
              ...
              <div className="space-y-3 mb-4">
                <div className="flex justify-between text-gray-600">
                  ...
                  <span>LKR {cart?.totalMrpPrice?.toLocaleString() || 0}</span>
                </div>
                <div className="flex justify-between text-green-600">
                  <span>Discount</span>
                  <span>-LKR {cart?.discount?.toLocaleString() || 0}</span>
                </div>
                {cart?.couponPrice > 0 && (
                  <div className="flex justify-between text-green-600">
                    <span>Coupon Discount</span>
                    <span>-LKR {cart?.couponPrice?.toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between text-gray-600">
                  <span>Delivery</span>
              ...
              <div className="border-t pt-4 mb-6">
                <div className="flex justify-between text-lg font-semibold">
                  <span>Total</span>
                  <span>LKR {cart?.totalSellingPrice?.toLocaleString() || 0}</span>
                </div>
                {cart?.discount > 0 && (
                  <p className="text-sm text-green-600 mt-1">
                    You're saving LKR {cart?.discount?.toLocaleString()}!
                  </p>
                )}
              </div>
